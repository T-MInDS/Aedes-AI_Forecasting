...
def get_finetuning_data(config: dict):
    ...
    # train: <=2015 (drop first 180 rows for warmup), val: 2016, test: >=2017
    train = data[data.Year <= 2015].iloc[180:, :].reset_index(drop=True)
...
def plot_finetune_history(history, out_dir):
    ...
    loss_path = os.path.join(out_dir, 'gru_avg_temp_finetune_history.png')
    fig.savefig(loss_path, dpi=300, bbox_inches='tight')
    plt.close(fig)

    # csv
    csv_path = os.path.join(out_dir, 'gru_avg_temp_finetune_history.csv')
...
def main():
    ...
    _, _, _, model_files_path, raw_mols_path = gen_utils.load_paths(fpaths_cfg)
    ...
    training_output_path = gen_utils.load_output_paths(fpaths_cfg)
    training_output_path = os.path.expanduser(training_output_path)
    os.makedirs(training_output_path, exist_ok=True)

    finetune_cfg_path = os.path.join(model_files_path, 'finetune_config.json')
    finetuned_model_path = os.path.join(model_files_path, 'gru_avg_temp_finetuned.keras')

    # Load base or existing finetuned artifacts
    if os.path.exists(finetune_cfg_path) and os.path.exists(finetuned_model_path):
        # Load existing finetuned config + model + scaler
        finetune_model, finetune_config, scaler = gen_utils.load_finetune_model_fils(
            model_files_path)
        base_or_ft_model = finetune_model
        cfg = finetune_config
    else:
    ...
    train, val, test = get_finetuning_data(cfg)
    ...
    history, finetuned_model = train_finetune_model(
        base_or_ft_model, X_train, y_train, X_val, y_val, cfg
    )
    plot_finetune_history(history, training_output_path)

    # Save model
    save_path = os.path.expanduser(cfg['files']['model'])
