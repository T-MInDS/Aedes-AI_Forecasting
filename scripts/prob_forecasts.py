...
def process_negbin_samples(samples):
    forecasts = []
    for sample in samples:
        sample = scale_point_predictions(sample)
        ...
        ns, mean_p = negbin_params(observed, forecast['Point_predictions'])
        forecast = forecast.copy()
        forecast['ns'] = ns
        forecast['mean_p'] = mean_p

        sample = pd.concat([observed, forecast], ignore_index=True).reset_index(drop=True)
        forecasts.append(sample)
...
def main():
    ...
    if True:
        ...
        negbin_forecasts = process_negbin_samples(negbin)
