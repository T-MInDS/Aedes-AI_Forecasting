...
def format_lcd(data_path, loc):
    ...
    data.drop(columns=['STATION', 'DATE',
              'REPORT_TYPE', 'SOURCE'], inplace=True)
    # Replace trace amounts of precipitation
    data[['DailyPrecipitation', 'HourlyPrecipitation']] = data[[
        'DailyPrecipitation', 'HourlyPrecipitation']].replace('T', 0)
    # Ensure columns are numeric
    cols = ['DailyAverageDryBulbTemperature', 'DailyAverageRelativeHumidity', 'DailyPrecipitation',
    ...
    data_daily['DailyAverageDryBulbTemperature'] = data_daily['DailyAverageDryBulbTemperature'].fillna(
        data_daily['HourlyDryBulbTemperature'])
    data_daily['DailyAverageRelativeHumidity'] = data_daily['DailyAverageRelativeHumidity'].fillna(
        data_daily['HourlyRelativeHumidity'])
    data_daily['DailyPrecipitation'] = data_daily['DailyPrecipitation'].fillna(
        data_daily['HourlyPrecipitation'])
    ...
    # Rename columns
    data_daily.rename(columns={'DailyAverageDryBulbTemperature': 'Avg_Temp',
                      'DailyAverageRelativeHumidity': 'Humidity', 'DailyPrecipitation': 'Precip_in'}, inplace=True)
    ...
    data_daily['Ref'] = 0
    data_daily = data_daily[['Location', 'Year', 'Month',
                             'Day', 'Avg_Temp', 'Precip_in', 'Humidity', 'Ref']]
    data_daily['Datetime'] = pd.to_datetime(
        data_daily[['Year', 'Month', 'Day']])
    data_cols = ['Avg_Temp', 'Precip_in', 'Humidity']

    data_daily = impute_vals(data_daily, data_cols)
    data_daily = interpolate_outliers(
        data_daily, data_cols, [], noise_std=0.05, threshold=100)

    data_daily.to_csv('../data/Weather/{}.csv'.format(loc), index=False)

    data_daily['Precip'] = 25.4 * data_daily['Precip_in']
    data_daily = data_daily[['Location', 'Year', 'Month',
                             'Day', 'Avg_Temp', 'Precip', 'Humidity', 'Ref']]
...
def collect_ceiba():
    ...
    full_idx = pd.date_range(start=asos.index.min(),
                             end=asos.index.max(), freq='D')
    ...
    coop_crlp['avg_temp_f'] = (
        coop_crlp['min_temp_f'] + coop_crlp['max_temp_f']) / 2
    ...
    coop_ngbp['precip_in'] = coop_ngbp['precip_in'].fillna(
        coop_crlp['precip_in'])
    coop_ngbp.drop(columns=['day', 'max_temp_f', 'min_temp_f',
                   'min_rh', 'avg_rh', 'max_rh'], inplace=True)
    coop_crlp.drop(columns=['station', 'precip_in'], inplace=True)

    full = pd.merge(coop_ngbp, coop_crlp, left_index=True,
    ...
    asos.drop(columns=['station', 'day', 'max_temp_f', 'min_temp_f',
              'precip_in', 'avg_temp_f', 'avg_rh', 'min_rh', 'max_rh'], inplace=True)

    full = pd.merge(full, asos, left_index=True, right_index=True, how='left')
    full.rename(columns={'precip_in': 'Precip_in',
                'avg_temp_f': 'Avg_Temp'}, inplace=True)
...
def impute_vals(data, data_cols, exclude_ranges=[],  noise_std_frac=0.05):
    ...
    for idx, row in filled.iterrows():
        ...
        if row[data_cols].isnull().any():
            ...
            missing_cols = row[data_cols][row[data_cols].isnull()
                                          ].index.tolist()
            # columns with valid data
            valid_cols = row[data_cols][row[data_cols].notnull()
                                        ].index.tolist()
            ...
            if not candidates.empty:
                ...
                distances = euclidean_distances(
                    row_vals, candidate_vals).flatten()
                ...
                for col in missing_cols:
                    ...
                    noise = np.random.normal(
                        loc=0, scale=noise_std_frac*abs(value))
...
def interpolate_outliers(data, data_cols, exclude_ranges=[], noise_std=0.05, threshold=3):
    ...
    exclusion_ranges = [(pd.to_datetime(start), pd.to_datetime(end))
    ...
    for col in data_cols:
        ...
        noise = np.random.normal(
            loc=0.0, scale=noise_std*data[col].mean(), size=len(nan_indices))
...
def cleanDailySummaries(data, loc):
    ...
    filled = filled[['Location', 'Datetime', 'Year', 'Month',
                     'Day', 'Avg_Temp', 'Precip_in', 'Humidity', 'Ref']]
