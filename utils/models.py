...
def ff_model(input_shape):
    ...
    c1 = tf.keras.layers.Conv1D(
        64, 3, activation='relu', data_format='channels_last')(xin)
    c2 = tf.keras.layers.Conv1D(
        64, 3, activation='relu', data_format='channels_last')(c1)
    ...
    xout = tf.keras.layers.Dense(
        1, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(0.001))(h4)
...
def lstm_model(input_shape):
    ...
    xout = tf.keras.layers.Dense(
        1, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(0.001))(r2)
