"""
This file is slightly modified from the Aedes-AI package at
//github.com/T-MInDS/Aedes-AI
"""
...
Returns:
  results: Array with the following columns: County, Year, Month, Day,
            Ref prediction for spatiotemporal location, Model prediction for spatiotemporal location
...
def gen_preds(model, data, data_shape, scaler=None, fit_scaler=False, smooth=True):
    ...
    if ref==False:
        ...
        results=np.concatenate([locs,np.reshape(data_ref[:,-1],(len(data_ref),1)),
                       np.reshape(data_nn[:,-1],(len(data_nn),1))], axis=1)
